...
struct Outputs {
    std::vector<double> grad;    // M*2
    std::vector<double> hess;    // M*4
    std::vector<double> energy;  // M
    std::vector<double> mindet;  // M
    std::vector<double> e2;      // M  (patch_energy_mindet)
    std::vector<double> md2;     // M
    std::vector<double> delta;   // M*2
};
...
void run_patch(sycl::queue& q, std::size_t M, Outputs& out)
{
    ...
    std::vector<double> h_X(M * kMaxNodes * 2);
    std::vector<int> h_gc(M * kMaxP), h_gn0(M * kMaxP), h_gn1(M * kMaxP), h_role(M * kMaxP);
    std::vector<double> h_s0(M * kMaxP), h_s1(M * kMaxP), h_W(M * kMaxP * 4), h_J(M * kMaxP * 24),
      h_params(M * 12);
    for (std::size_t k = 0; k < M; ++k) {
        const auto& s = golden::kPatchSamples[k];
        h_P[k] = s.P;
        h_tag[k] = s.tag;
        for (int i = 0; i < kMaxNodes * 2; ++i) h_X[k * kMaxNodes * 2 + i] = s.X[i];
        for (int i = 0; i < kMaxP; ++i) {
            h_gc[k * kMaxP + i] = s.gc[i];
            h_gn0[k * kMaxP + i] = s.gn0[i];
            h_gn1[k * kMaxP + i] = s.gn1[i];
            h_role[k * kMaxP + i] = s.role[i];
            h_s0[k * kMaxP + i] = s.s0[i];
            h_s1[k * kMaxP + i] = s.s1[i];
        }
        for (int i = 0; i < kMaxP * 4; ++i) h_W[k * kMaxP * 4 + i] = s.W_inv[i];
        for (int i = 0; i < kMaxP * 24; ++i) h_J[k * kMaxP * 24 + i] = s.J[i];
        for (int i = 0; i < 12; ++i) h_params[k * 12 + i] = s.params[i];
    }

    // Device USM.
    auto* d_P = sycl::malloc_device<int>(M, q);
    auto* d_tag = sycl::malloc_device<int>(M, q);
    auto* d_X = sycl::malloc_device<double>(M * kMaxNodes * 2, q);
    ...
    auto* d_s0 = sycl::malloc_device<double>(M * kMaxP, q);
    auto* d_s1 = sycl::malloc_device<double>(M * kMaxP, q);
    auto* d_W = sycl::malloc_device<double>(M * kMaxP * 4, q);
    auto* d_J = sycl::malloc_device<double>(M * kMaxP * 24, q);
    auto* d_params = sycl::malloc_device<double>(M * 12, q);
    auto* d_grad = sycl::malloc_device<double>(M * 2, q);
    auto* d_hess = sycl::malloc_device<double>(M * 4, q);
    auto* d_energy = sycl::malloc_device<double>(M, q);
    auto* d_mindet = sycl::malloc_device<double>(M, q);
    auto* d_e2 = sycl::malloc_device<double>(M, q);
    auto* d_md2 = sycl::malloc_device<double>(M, q);
    auto* d_delta = sycl::malloc_device<double>(M * 2, q);

    q.memcpy(d_P, h_P.data(), M * sizeof(int));
    q.memcpy(d_tag, h_tag.data(), M * sizeof(int));
    q.memcpy(d_X, h_X.data(), h_X.size() * sizeof(double));
    ...
    q.memcpy(d_s0, h_s0.data(), h_s0.size() * sizeof(double));
    q.memcpy(d_s1, h_s1.data(), h_s1.size() * sizeof(double));
    q.memcpy(d_W, h_W.data(), h_W.size() * sizeof(double));
    q.memcpy(d_J, h_J.data(), h_J.size() * sizeof(double));
    q.memcpy(d_params, h_params.data(), h_params.size() * sizeof(double));
    q.wait();
    ...
    q.parallel_for(sycl::range<1>(M),
                   [=](sycl::id<1> idx) {
                       ...
                       PatchView v {d_P[k],
                                    ...
                                    &d_role[k * kMaxP],
                                    &d_J[k * kMaxP * 24]};
                       const double* X = &d_X[k * kMaxNodes * 2];
                       ...
                       // Tangent-reduced Newton step, fed the patch's own grad/hess. The moving
    ...
    q.memcpy(out.grad.data(), d_grad, M * 2 * sizeof(double));
    q.memcpy(out.hess.data(), d_hess, M * 4 * sizeof(double));
    q.memcpy(out.energy.data(), d_energy, M * sizeof(double));
    q.memcpy(out.mindet.data(), d_mindet, M * sizeof(double));
    q.memcpy(out.e2.data(), d_e2, M * sizeof(double));
    q.memcpy(out.md2.data(), d_md2, M * sizeof(double));
    q.memcpy(out.delta.data(), d_delta, M * 2 * sizeof(double));
    q.wait();
    ...
    for (const auto& dev : devices) {
        ...
        test("patch on " + name) = [&] {
            ...
            for (std::size_t k = 0; k < M; ++k) {
                const auto& s = golden::kPatchSamples[k];
                for (int i = 0; i < 2; ++i)
                    expect(close(out.grad[k * 2 + i], s.grad[i], kTolGH))
                      << std::format("patch {} grad[{}]: {} vs {}",
                                     ...
                                     s.grad[i]);
                for (int i = 0; i < 4; ++i)
                    expect(close(out.hess[k * 4 + i], s.hess[i], kTolGH))
                      << std::format("patch {} hess[{}]: {} vs {}",
                                     k,
                                     i,
                                     out.hess[k * 4 + i],
                                     s.hess[i]);
                expect(close(out.energy[k], s.energy, kTolE))
                  << std::format("patch {} energy: {} vs {}", k, out.energy[k], s.energy);
                expect(close(out.mindet[k], s.mindet, kTolE))
                  << std::format("patch {} mindet: {} vs {}", k, out.mindet[k], s.mindet);
                expect(close(out.e2[k], s.energy, kTolE))
                  << std::format("patch {} energy(cheap): {} vs {}", k, out.e2[k], s.energy);
                expect(close(out.md2[k], s.mindet, kTolE))
                  << std::format("patch {} mindet(cheap): {} vs {}", k, out.md2[k], s.mindet);
                for (int i = 0; i < 2; ++i)
                    expect(close(out.delta[k * 2 + i], s.delta[i], kTolGH))
