                  ...
                  std::vector<double>& tang)
{
    ...
    double* d_params = sycl::malloc_device<double>(M * 12, q);
    double* d_p = sycl::malloc_device<double>(M * 2, q);
    double* d_proj = sycl::malloc_device<double>(M * 2, q);
    double* d_tang = sycl::malloc_device<double>(M * 2, q);

    std::vector<int> h_tag(M);
    std::vector<double> h_params(M * 12), h_p(M * 2);
    ...
    q.memcpy(d_params, h_params.data(), M * 12 * sizeof(double));
    q.memcpy(d_p, h_p.data(), M * 2 * sizeof(double));
    q.wait();
    ...
    q.memcpy(proj.data(), d_proj, M * 2 * sizeof(double));
    q.memcpy(tang.data(), d_tang, M * 2 * sizeof(double));
    q.wait();
...
int main()
{
    ...
    for (const auto& dev : devices) {
        ...
        test("geometry on " + name) = [&] {
            ...
            std::vector<double> proj(M * 2), tang(M * 2);
            run_geometry(q, M, proj, tang);
            for (std::size_t k = 0; k < M; ++k) {
                const auto& s = golden::kGeoSamples[k];
                for (int i = 0; i < 2; ++i) {
                    expect(close(proj[k * 2 + i], s.proj[i], kTol))
                      << std::format("sample {} (tag {}) proj[{}]: {} vs {}",
                                     ...
                                     proj[k * 2 + i],
                                     s.proj[i]);
                    expect(close(tang[k * 2 + i], s.tang[i], kTol))
