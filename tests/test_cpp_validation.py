"""Binding-boundary validation: malformed contexts/X must raise a Python
exception, not crash the interpreter with device-side UB.

These exercise the host-side ``validate_context`` / ``checked_num_nodes`` /
dimension guards in ``src/bindings.cpp``, driven through the structured
block-Jacobi entry point (``cpp_core.cpp_structured_sweep``). Skips if the C++
core is not built.
"""

from __future__ import annotations

import copy

import numpy as np
import pytest


def _has_cpp() -> bool:
    try:
        from egg._cpp import cpp_core  # noqa: F401

        return True
    except Exception:
        return False


pytestmark = pytest.mark.skipif(
    not _has_cpp(),
    reason="egg._cpp.cpp_core not built (requires cmake build)",
)


def _valid_inputs():
    """A valid (ctx_arrays, structured, X_flat) triple from a 4x4 demo grid."""
    from egg.smoothing.cpp_backend import (
        build_block_structured_context, structured_arrays)
    from egg.smoothing.solver import build_sweep_context
    from egg.smoothing.targets import IdentityTarget
    from egg.topology.builder import TopologyBuilder

    builder = TopologyBuilder(d=2)
    for name, pos in [("A", (0.0, 0.0)), ("B", (4.0, 0.0)),
                      ("C", (4.0, 4.0)), ("D", (0.0, 4.0))]:
        builder.add_corner(name, pos, fixed=True)
    builder.add_block("main", ("A", "D", "B", "C"), (4, 4))
    topo = builder.build()
    topo.initialize_grid()
    grid = topo.grid
    ctx = build_sweep_context(grid, IdentityTarget(d=2))
    ctx_arrays = ctx.wire
    structured = structured_arrays(build_block_structured_context(grid))
    X_flat = np.ascontiguousarray(grid.global_nodes, dtype=np.float64).ravel()
    return ctx_arrays, structured, X_flat


def _sweep(ctx_arrays, structured, X_flat, *, dim=2):
    from egg._cpp import cpp_core

    return cpp_core.cpp_structured_sweep(
        ctx_arrays, structured, X_flat, 1, device="cpu", dim=dim)


def test_valid_inputs_do_not_raise():
    """Sanity: the unmodified inputs run a sweep cleanly."""
    ctx_arrays, structured, X_flat = _valid_inputs()
    _sweep(ctx_arrays, structured, X_flat)


def test_2d_shaped_X_raises():
    """An (N, 2) array (instead of flat (2N,)) must raise, not silently halve N."""
    ctx_arrays, structured, X_flat = _valid_inputs()
    with pytest.raises(Exception):
        _sweep(ctx_arrays, structured, X_flat.reshape(-1, 2))


def test_odd_length_X_raises():
    """An odd-length flat X (not 2 coords per node) must raise."""
    ctx_arrays, structured, X_flat = _valid_inputs()
    with pytest.raises(Exception):
        _sweep(ctx_arrays, structured, np.append(X_flat, 0.0))


def test_out_of_range_index_raises():
    """An out-of-range node index in a group must raise before any device work."""
    ctx_arrays, structured, X_flat = _valid_inputs()
    bad = copy.deepcopy(ctx_arrays)
    num_nodes = X_flat.shape[0] // 2
    bad["groups"][0]["gc"] = bad["groups"][0]["gc"].copy()
    bad["groups"][0]["gc"][0] = np.int32(num_nodes + 999)
    with pytest.raises(Exception):
        _sweep(bad, structured, X_flat)


def test_out_of_range_role_raises():
    """A role outside [-1, kNbrsPerCorner] must raise (it indexes J columns via
    c0 = kDim*role, so an out-of-range role would read past the sample's J block)."""
    ctx_arrays, structured, X_flat = _valid_inputs()
    bad = copy.deepcopy(ctx_arrays)
    bad["groups"][0]["role"] = bad["groups"][0]["role"].copy()
    bad["groups"][0]["role"][0] = np.int32(3)  # > kNbrsPerCorner (2 in 2D)
    with pytest.raises(Exception):
        _sweep(bad, structured, X_flat)


def test_out_of_range_energy_stencil_index_raises():
    """An out-of-range index in the energy stencil must also raise."""
    ctx_arrays, structured, X_flat = _valid_inputs()
    bad = copy.deepcopy(ctx_arrays)
    num_nodes = X_flat.shape[0] // 2
    bad["energy_stencil"]["gn0"] = bad["energy_stencil"]["gn0"].copy()
    bad["energy_stencil"]["gn0"][0] = np.int32(num_nodes + 999)
    with pytest.raises(Exception):
        _sweep(bad, structured, X_flat)


def test_dim3_rejects_2d_shaped_context():
    """dim=3 is a real dispatch now; feeding it a d=2-shaped context must fail
    the pre-kernel validation, not reach the device."""
    ctx_arrays, structured, X_flat = _valid_inputs()
    with pytest.raises(Exception):
        _sweep(ctx_arrays, structured, X_flat, dim=3)


def test_dim4_raises_value_error():
    """dim=4 is outside the supported {2, 3} set and raises ValueError."""
    ctx_arrays, structured, X_flat = _valid_inputs()
    with pytest.raises(ValueError, match="dimension must be 2 or 3"):
        _sweep(ctx_arrays, structured, X_flat, dim=4)
