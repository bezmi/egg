...
def _assert_converges(ctx, grid, X0, n_sweeps):
    ...
    np.testing.assert_allclose(
        e_j[-1], _energy(X_j, ctx), rtol=real_tol(0.0), atol=real_tol(1e-9)
    )
    np.testing.assert_allclose(
        m_j[-1],
        grid_min_det(X_j, ctx.energy_stencil),
        rtol=real_tol(0.0),
        atol=real_tol(1e-9),
    )
    return X_j
...
def test_block_jacobi_reaches_analytic_minimiser():
    ...
    X_j, _e, m_j = cpp_structured_sweep(ctx, grid, X0, 3000, device="cpu")
