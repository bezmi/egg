...
def test_respace_line_graded_tail_when_layers_overshoot():
    ...
    new = _respace_line(pts, first_height=0.005, growth=1.3, n_layers=8,
                        max_height=None)
    sp = np.diff(new[:, 0])
    assert sp[0] == pytest.approx(0.005, rel=1e-12)
    ...
    assert tail_ratios == pytest.approx(np.full(len(tail_ratios),
                                                tail_ratios[0]), rel=1e-6)
    assert new[-1, 0] == 0.5
...
def test_straightening_keeps_far_boundary_row_on_geometry():
    ...
    topo.boundary_layer_specs = {id(ents["circle"]): dict(
        first_height=0.02, growth=1.3, n_layers=8,
        max_height=None, tangential_spacing=None)}
    grid = topo.initialize_grid()
    enforce_boundary_layer_spacing(grid)
    ...
    for blk, ent in (("e_s", "bottom"), ("e_e", "right"),
                     ("e_n", "top"), ("e_w", "left")):
        bi = names.index(blk)
