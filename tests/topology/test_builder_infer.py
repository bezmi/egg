...
class TestCompassAddBlock:
    ...
    def test_fixed_taken_from_object(self):
        ...
        b.add_block(sw=Vector3(0, 0, fixed=True), se=e.place_node(1.0),
                    nw=Vector3(0, 4), ne=e.place_node(0.5, fixed=True),
                    res=(2, 2))
    ...
    def test_mixing_forms_rejected(self):
        ...
        with pytest.raises(ValueError):
            b.add_block(sw=Vector3(0, 0), se=Vector3(1, 0),
                        nw=Vector3(0, 1), ne=Vector3(1, 1))
        with pytest.raises(TypeError):
            b.add_block(sw=(0, 0), se=(1, 0), nw=(0, 1), ne=(1, 1),
                        res=(1, 1))

    def test_positional_form_unchanged(self):
        b = TopologyBuilder(d=2)
        for n, p in [("sw", (0, 0)), ("se", (1, 0)), ("nw", (0, 1)),
                     ("ne", (1, 1))]:
            b.add_corner(n, p)
...
class TestAssociateInference:
    ...
    def test_explicit_association_not_duplicated(self):
        g = _wall_edges()
        b = TopologyBuilder(d=2)
        b.add_block("blk", res=(2, 2),
                    sw=g["bottom"].place_node(0.0),
                    se=g["bottom"].place_node(1.0),
                    nw=Vector3(0, 4), ne=Vector3(4, 4))
    ...
    def test_connected_faces_not_associated(self):
        ...
        b.add_block("lo", res=(2, 2), sw=Vector3(0, -1), se=Vector3(4, -1),
                    nw=m0, ne=m1)
        b.add_block("hi", res=(2, 2), sw=m0, se=m1,
                    nw=Vector3(0, 4), ne=Vector3(4, 4))
        topo = b.build()
        assert len(topo.interface_connections) == 1
        faces = {(a.face.block_name, a.face.axis, a.face.side)
