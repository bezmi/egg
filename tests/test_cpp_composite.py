...
from tests.real_tol import real_tol  # noqa: E402  (after the cpp skip guard)
...
def _make_composite(kind: str):
    ...
    if kind == "lines":
        return Polyline(
            [Line(Vector3(0, 0), Vector3(0, 2)), Line(Vector3(0, 2), Vector3(0, 4))]
        )
    if kind == "arcs":
        ...
        return Polyline(
            [
                Arc(Vector3(0, 0), Vector3(0, 2), Vector3(-10, 1)),
...
def test_composite_device_relaxes_on_curve(kind):
    ...
    X_cpp, _e, _m = cpp_structured_sweep(ctx, grid, X0.copy(), 5, device="cpu")
