...
Starts from a deliberately *folded* circle-in-rectangle topology (rough inner
O-ring corners give ``min det A < 0``) and runs the full pipeline:

    TFI init → boundary snap → δ-continuation untangle → TMOP quality opt → valid
...
def main():
    p = argparse.ArgumentParser(description="folded → untangled → smoothed.")
    p.add_argument(
        "--plot-live",
        action="store_true",
    ...
    print("=" * 56)
    print("folded circle-in-rectangle → untangle → smooth")
    print("=" * 56)

    topo, ents = build_circle_in_rectangle(rough=True)
    ...
    if a.plot_live:
        ...
        if a.plot_energy:
            ...
            ax[1].set(title="min det A (untangle+TMOP)", xlabel="chunk")
            plt.tight_layout()
            plt.show()
