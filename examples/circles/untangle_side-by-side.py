"""side-by-side demo: twin-circle untangle + per-wall boundary layers.

Usage::

    uv run untangle_side-by-side.py [--plot-live] [--plot-grid]
...
def main():
    p = argparse.ArgumentParser(description="twin-circle untangle + BL.")
    p.add_argument(
        "--plot-live",
        action="store_true",
    ...
    print("=" * 56)
    print("side-by-side: twin-circle untangle + per-wall BL")
    print("=" * 56)
    ...
    topo, ents = build_twin_circle(rough=True, bl=bl, R=a.resolution)
